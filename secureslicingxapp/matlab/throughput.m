...
plot(time_intervals, smoothened_data1, 'r-', 'DisplayName', 'UE rate');
hold on;
% Plot the smoothed data for the second dataset
plot(time_intervals, smoothened_data2, 'g-', 'DisplayName', 'UE rate');

plot(time_intervals, smoothened_data3, 'g-', 'DisplayName', 'UE rate');


xlabel('Time (seconds)');
ylabel('Downlink data rate (Mbps)');
