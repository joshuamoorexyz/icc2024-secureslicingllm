...
bool App::handle(e2sm::kpm::KpmIndication *kind)
{
    ...
    std::ofstream outfile("elapsed_times.txt", std::ios::app);
	...
	mdclog_write(MDCLOG_DEBUG, "Start time: %s", std::ctime(&start_time_t));
...
for (auto it = report->ues.begin(); it != report->ues.end(); ++it) {
		...
		if(COUNTER1 == 10 || COUNTER2 == 10){
			...
			{	
				...
				if(avg_tx_1 >= tx_threshold){
					...
					//calculate sleep proportion before tx_threshold is 10x
					double proportion_over_threshold = 0.0;
					proportion_over_threshold = static_cast<double>(avg_tx_1 - tx_threshold) / tx_threshold;
        			proportion_over_threshold = std::min(1.0, std::max(0.0, proportion_over_threshold));

					// Map the proportion to a sleep duration between 1000 and 10000 milliseconds (1 to 10 seconds)
    				int min_sleep_duration_ms = 1000;
    				int max_sleep_duration_ms = 10000;
    				int sleep_duration_ms = min_sleep_duration_ms + static_cast<int>(proportion_over_threshold * (max_sleep_duration_ms - min_sleep_duration_ms + 1));
					...
					//sleep for some time proportional to the amount of pkts over the threshold it is
    				// Sleep for the calculated duration in milliseconds
					mdclog_write(MDCLOG_DEBUG, "Passing though latency test");
					if(latencytest)
					{
    					std::this_thread::sleep_for(std::chrono::milliseconds(sleep_duration_ms));
					}
					mdclog_write(MDCLOG_DEBUG, "Passed through latency test");
			...
			{	
				...
				if(avg_tx_2 >= tx_threshold)
				{
					//calculate sleep proportion before tx_threshold is 10x
					double proportion_over_threshold = 0.0;
					proportion_over_threshold = static_cast<double>(avg_tx_2 - tx_threshold) / tx_threshold;
        			proportion_over_threshold = std::min(1.0, std::max(0.0, proportion_over_threshold));
	...
	// Write the elapsed time and current date/time to the file
    outfile << "Elapsed time: " << elapsed_time.count() << " microseconds"
                << " at " << std::ctime(&current_time) << std::endl;


	// Log the elapsed time and current date/time using mdclog
    mdclog_write(MDCLOG_DEBUG, "Elapsed time: %ld microseconds at %s", elapsed_time.count(), std::ctime(&current_time));
